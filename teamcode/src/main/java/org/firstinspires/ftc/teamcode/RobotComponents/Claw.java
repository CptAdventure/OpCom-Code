...
public class Claw {
    ...
    public double extend (boolean extend, boolean retract) { // Boolean input (autonomous)
        ...
        clawPosition += ((nanoTime() - oldTime) != 0) ? this.extend.getPower() / (nanoTime() - oldTime) * 100000 : 0; // Crude position from here to return
        oldTime = nanoTime();
    ...
    public double extend (boolean extend, boolean retract, boolean override) { // Same as previous, but override switches
        ...
        clawPosition += ((nanoTime() - oldTime) != 0) ? this.extend.getPower() / (nanoTime() - oldTime) * 100000 : 0;
        oldTime = nanoTime();
        if(cfSwitch.getState()) {
            clawPosition = 17.5;
        }
    ...
    public double extend (double extendPower) { // Number input (Joystick)
        ...
        clawPosition += ((nanoTime() - oldTime) != 0) ? this.extend.getPower() / (nanoTime() - oldTime) * 100000 : 0; // Crude position from here on down
        oldTime = nanoTime();
        if(cfSwitch.getState()) {
            clawPosition = 17.5;
        }
